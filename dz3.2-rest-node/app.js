...
app.get("/employee/:id", function (req, res) {
  ...
  if (!emp_id) {
    return res
      .status(400)
      .send({ error: true, message: "Please provide emp_id" });
  }
  dbConn.query(
...
app.post("/employee", function (req, res) {
  ...
  dbConn.query(
    ...
    [birth_date, first_name, first_name, gender, hire_date],
    function (error, results, fields) {
      if (error) throw error;
      return res.send({
        error: false,
...
app.put("/employee", function (req, res) {
  ...
  if (!emp_no || !employee) {
    return res
      .status(400)
      .send({ error: employee, message: "Please provide employee and emp_no" });
  }
  dbConn.query(
    "UPDATE employees SET first_name = ?, last_name = ?, birth_date = ?, gender =?, hire_date = ? where emp_no = ?",
